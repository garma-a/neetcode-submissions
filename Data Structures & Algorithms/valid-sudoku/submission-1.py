class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        row_len, col_len = 9, 9
        for current_row in range(0, row_len, 3):
            for current_col in range(0, col_len, 3):
                if (
                    not self.isValidCol(board, current_col)
                    or not self.isValidSquare(board, current_row, current_col)
                    or not self.isValidRow(board[current_row])
                ):
                    return False
